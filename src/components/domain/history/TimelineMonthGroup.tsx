    ...
    <div className="relative">
      {!isFirstGroup && (
        <div className="pl-16 mb-20">
          <Label
            name={yearMonth}
            styleClass="rounded-md border-stroke-neutral-subtle bg-bg-neutral-weak px-10 py-4 text-11 text-fg-brand"
          />
        </div>
      )}
            ...
            <React.Fragment key={item.historyId}>
              {showDivider && (
                <div className="relative py-4">
                  <div className="absolute left-0 right-0 h-1 bg-stroke-neutral-subtle" />
                </div>
              )}
              <TimeLineItem
                date={item.createdAt}
      ...
      {!isLastGroup && <div className="my-20" />}
    </div>
  );
}

      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="font-bold text-13 text-fg-neutral">
            <th className="text-left py-17 px-16 ">문의 제목</th>
            <th className="text-left py-17 px-16">등록일</th>
            <th className="text-left py-17 px-16">답변</th>
          </tr>
        </thead>
        <tbody>
          {inquireList.map((item) => (
            ...
            >
              <td className="py-15 px-16 text-13 text-fg-neutral">
                <div className="text-fg-neutral line-clamp-2 break-words">
                  {item.title && truncStr(item.title, 20)}
                </div>
              </td>
              <td className="py-15 px-16 text-13 text-fg-neutral whitespace-nowrap">
                {format(new Date(item.createAt), 'yyyy.MM.dd')}
              </td>
              <td className="py-15 px-16">

...
function CurationCardListContent() {
  const { data, isLoading, isError } = useCurationsQuery();

  if (isLoading) {
    return (
      <div className="space-y-7 px-5 pb-navbar">
        <div className="h-96 animate-pulse rounded-md bg-sectionWhite" />
        <div className="h-96 animate-pulse rounded-md bg-sectionWhite" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-7 px-5 pb-navbar">
      {data.map((event, index) => (
        <CurationCard key={event.id} curation={event} priority={index === 0} />
      ))}
    </div>
  );
}
...
function CurationCardList({ currentTab }: { currentTab: CurationTabId }) {
  ...
  return <CurationCardListContent />;
}

...
export function ProgramScheduleItem({ program }: ProgramScheduleItemProps) {
  ...
  return (
    <article className="rounded-xl border border-stroke-neutral-subtle p-16">
      <span className="inline-flex rounded-full bg-bg-brand-weak px-8 py-4 text-11 font-bold text-fg-brand">
        {formatProgramType(program.type)}
      </span>
      <h3 className="mt-8 text-16 font-extrabold text-fg-neutral">
        {program.name}
      </h3>

      <div className="mt-12 space-y-8 text-13 font-medium text-fg-neutral-muted">
        {program.venue && (
        ...
        {program.applicationUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex gap-8 font-bold text-fg-brand"
          >
      ...
      {program.description && (
        <p className="mt-16 whitespace-pre-line text-13 font-medium leading-[1.7] text-fg-neutral">
          {program.description}
        </p>
      )}
      ...
      {program.whiskies && program.whiskies.length > 0 && (
        <section className="mt-24">
          ...
          <div className="mt-12 divide-y divide-stroke-neutral-basement border-t border-stroke-neutral-basement">

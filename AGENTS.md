...
- 기존 UI 요소를 추가·재배치하거나 조합할 때는 별도 요구가 없으면 컴포넌트의 높이, 너비, 패딩, 간격, 글자와 아이콘 크기를 유지한다. 배치 변경을 이유로 임의로 확대·축소하지 않으며, 크기 변경이 필요하면 화면 영향과 이유를 먼저 설명하고 사용자와 논의한다.
...
- PR에는 이슈를 자동 종료하는 키워드 없이 마크다운 링크로 `[#<번호>](https://github.com/bottle-note/workspace/issues/<번호>)` 형식으로 관련 이슈를 연결한다. 다른 영역의 의존성이 있으면 현재 구현 범위와 후속 작업을 PR에 구분해 적는다.
